...
export default class CommentListComment extends React.Component {
  constructor(props) {
    super(props);
  ...
  renderCommentOptions() {
    return (
      <span>
        <span onClick={this.triggerDelete} className={Styles.commentLink}>Delete</span><span style={{marginLeft: '10px'}}>&middot;</span>
        { !this.state.replying && <span onClick={this.triggerReply} className={Styles.commentLink}>Reply</span> }
        { !this.state.showReplies && (this.props.commentObject.replies.length > 0) && <span style={{marginLeft: '10px'}}><span>&middot;</span><span onClick={this.triggerReply} className={Styles.commentLink}>Show {this.props.commentObject.replies.length} Replies</span></span> }
        { this.state.showReplies && <span onClick={this.hideReply} className={Styles.commentLink}>Hide Replies</span> }
      </span>
    );
  }
        ...
        <div className={Styles.commentWrapper} onMouseEnter={this.mouseEnter} onMouseLeave={this.mouseLeave}>
          <div>
            <span className={Styles.profilePictureWrapper}>
              ...
              { true && <div className={Styles.onlineUser}></div> }
            </span>
          </div>
